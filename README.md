...
**预览**：[https://carsales.zhangzhicheng.info/](https://carsales.zhangzhicheng.info/)
...
脚本会创建数据库 `car_sales` 及所需表。若你改了库名/账号，请与下一步 `.env` 保持一致。Docker 若沿用旧 `mysql_data` 卷且表结构已变，需备份后删卷重建，再「刷新全部数据」拉全量（见 `docker/README.md`）。
